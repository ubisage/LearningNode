...
exports.postAddProduct = (req, res, next) => {
  ...
  const product = new Product({title:title, price, description, imageUrl,userId:req.user});
...
exports.getProducts = (req, res, next) => {
  // Product.find()
  // .select('title price -_id') response will have title and price, _id is excluded(Note : id has to be manually excluded becuase its explicitly included)
  // .populate('userId', name).then((products) => { //will get u all data not only users id butusers name and email|| if u add 2nd argument it will select and put only name and _id in return
